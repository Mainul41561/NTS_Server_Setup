...
This project demonstrates a simple and secure Network Time Security (NTS) lab environment using chrony on both the server and client systems.
...
![NTS Server Setup Screenshot](ChatGPT%20Image%20May%2017%2C%202026%2C%2010_52_22%20AM.png)

### ⚙️ Prerequisites

* Must have **VirtualBox** and **Vagrant** installed

---

## Lab Environment Setup

Change Directory to Your VagrantFile Directory
...
## Phase 01: Configure the NTS Server
Step 1: Generate the OpenSSL configuration to support the Subject Alternative Name (SAN) tracking:
...
```
Step 4: Deploy certificates to Chrony's system directory and apply Ubuntu-specific group permissions (_chrony):
```bash
...
```
Step 5: Overwrite the default configuration file cleanly using `tee`:
```bash
...
Execute these commands inside your host terminal to bridge the certificate across the isolated network layer:
```bash
...
```
---
## Phase 03:  Configure the NTS Client
Step 1: Install Chrony:
```bash
sudo apt update && sudo apt install -y chrony
```
Step 2: Stage the server's public certificate and apply the correct system permission context:
```bash
...
```
Step 3: Map the .local domain name directly inside the client's host database lookup file:
```bash
echo "192.168.56.10 nts-server.local" | sudo tee -a /etc/hosts
```
Step 4: Deploy the client configuration file using `tee` to prevent privilege execution errors:
```bash
cat << 'EOF' | sudo tee /etc/chrony/chrony.conf > /dev/null
